...
def Baseline(): #基线计算
   ...
   for i in range(len(pp)): 
       ...
       for j in range(len(pp[i])):
           tempbase=np.mean(pp[i][j][0:50])
           for ii in range(len(pp[i][j])):
               pp[i][j][ii]=tempbase-pp[i][j][ii]
           temp2=max(pp[i][j])
           ddd.append(temp2)
           bbb.append(pp[i][j].index(temp2))
       dd.append(ddd)
       bb.append(bbb)
       #baseline.append(tempbase)
   print('finish!')
...
def middleline(): #基线差 计算
   ...
   global bb #存最大值对应的采样点
   global dd #最大值
   patha1=filedialog.askopenfilename(title='导入基线', filetypes=[('数据文件', '*.pkl')])
   with open(patha1, 'rb') as file:
        mu_all = pickle.load(file)
   bb=[]
   dd=[]
   tempx=[[] for i in range(0,96)]
   for i in range(len(pp)): 
       bbb=[]
       ddd=[]
       for j in range(len(pp[i])):
           tempbase=np.mean(pp[i][j][0:50])
           tempa=mu_all[int(pp1[i][j])]-tempbase
           tempx[int(pp1[i][j])].append(tempa)
       dd.append(ddd)
       bb.append(bbb)
   for i in range(len(tempx)) :
       ...
       plt.title("%i通道基线差值统计"%i)
       plt.hist(tempx[i], bins = 100,range=(-10,10))
       plt.savefig('test2/'+str(i)+'.jpg')
      
   print('finish!')
       
def energy_single_count():
    global pp
    global pp1
    for i in range(0,96):
        temp=[]
        for j in range(len(pp)):
            for m in range(len(pp[j])):
                if  pp1[j][m]==i:
                    tempen=fun.single_energy__integrate(pp[nevent][i])
                    temp.append(tempen)
        matplotlib.use('Agg')
        matplotlib.pyplot.close()
        plt.figure(figsize=(10,4),dpi=150)
        plt.title("%i通道能损统计"%(i+1))
        plt.hist(temp, bins = 200,rwidth=0.8,align='left')
        plt.savefig('test3/'+str(i)+'.jpg')
        plt.close()
        
                    

    
def baseline_statistics(): #基线的统计并拟合
    from scipy.stats import norm
    global mu_all,std_all
    global temp_data_sort 
    mu_all=[] #存中心值
    std_all=[] #存sigma
    for i in range(0,96):
        temp=[]
        for m in range(len(temp_data_sort)):
            if temp_data_sort[m, 1]==i:
                temp.extend(temp_data_sort[m, 4:1027])
        ...
        plt.figure(figsize=(10,4),dpi=150)
        plt.title("%i通道基线统计"%i)
        #plt.hist(temp, bins = 200,range=(2500,3500),rwidth=0.8,align='left')
        
        n, bin0, patches=plt.hist(temp, bins = 300,range=(2900,3200))
        mu, std1 = norm.fit(temp)
        print(mu, std1)
        mu_all.append(mu)
        std_all.append(std)
        p = norm.pdf(bin0, mu, std1)*len(temp) * np.diff(bin0)[0]
        plt.plot(bin0, p, 'r', linewidth=2,label=f'mu = {mu:.2f}, std = {std1:.2f}')
        plt.legend()
        plt.savefig('test/'+str(i)+'.jpg')
        plt.close()
    plt.figure(figsize=(10,4),dpi=150)
    plt.title("基线分布统计")
    plt.hist(mu_all, bins = 20,rwidth=0.8,align='left')
    plt.show()
    with open(r'test\基线.pkl', 'wb') as file:
...
def plotting():
    ...
    def energy_spectrum(): #能谱
        ...
        n, bins, patches=plt.hist(energy, bins = 60,label='max')
        plt.title("能谱")
        plt.xlabel('Value')
        ...
        n2,e_all,channel_count=fun.energy_plotting_integrate(pp)
        
        plt.figure(figsize=(10,4),dpi=150)
        plt.scatter(energy, e_all, alpha=0.5,s=2)
        plt.title("关联图")
        plt.xlabel('max')
        plt.ylabel('integrate')
        pfit = np.polyfit(energy,e_all, 1)
        xfit = np.linspace(min(energy), max(energy), 2)
        yfit = np.polyval(pfit, xfit)
        plt.plot(xfit, yfit, 'r-', label= f'y = {pfit[0]:.2f}x + {pfit[1]:.2f}')
        #eqn = f'y = {pfit[0]:.2f}x + {pfit[1]:.2f}'
        #plt.text(min(energy), max(e_all), eqn, fontsize=12, color='red')
        plt.legend()
        plt.show() 
        plt.figure(figsize=(10,4),dpi=150)
        plt.scatter(channel_count, e_all, alpha=0.5,s=5)
    ...
    def location(): #画入射位置
        ...
        plt.show()
        fig = plt.figure()
        plt.title("x维起始位置采样点统计")
        plt.hist(x_c,bins =200,range=(0,1024),rwidth=0.8,align='left')
        plt.show()        
        fig2 = plt.figure()
        plt.title("y维入射通道统计")
        ...
        plt.show()        
        fig = plt.figure()
        plt.title("y维起始位置采样点统计")
        plt.hist(y_c,bins =200,range=(0,1024),rwidth=0.8,align='left')
        plt.show()        
        ...
        plt.xticks([4*i for i in range(0,14)], ['{:.2f}'.format(2.66*4*i) for i in range(0,14)])
        plt.yticks([4*i for i in range(0,14)], ['{:.2f}'.format(2.66*4*i) for i in range(0,14)])
       ...
       # a.patch.set_facecolor('black')
        plt.show()
        fig = plt.figure()
        plt.title("所有通道峰值采样点统计")
        plt.hist(all_c,bins =200,range=(0,1024),rwidth=0.8,align='left')
        plt.show()    
        '''
        fig = plt.figure()
...
up.pack()
up = Button(root, text="基线的筛除", font=('宋体', 12),command=Baseline) #Button是tkinter框架中的一种按钮，通过点击可以调用后端函数。
up.pack()
...
up = Button(root, text="基线的统计", font=('宋体', 12),command=baseline_statistics) #Button是tkinter框架中的一种按钮，通过点击可以调用后端函数。
up.pack()
up = Button(root, text="基线差统计", font=('宋体', 12),command=middleline) #Button是tkinter框架中的一种按钮，通过点击可以调用后端函数。
up.pack()
